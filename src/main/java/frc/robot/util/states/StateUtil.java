...
public class StateUtil {
  public static String toSuffix(StateValue value) {
    return "/" + value.getName()
        + LogUtil.toSuffix(value.getUnit() != null ? value.getUnit().symbol() : Value.symbol());
  }
  ...
  //   public abstract StateValue jerkValue();

  //   public default double jerk() {
  //     return StateUtil.getValueAsDouble(this.jerkValue());
  //   }
  // }

  // public interface PosVel_State extends Vel_State, Vel_State {}

  // public interface PosAcc_State extends Vel_State, Acc_State {}

  // public interface PosJerk_State extends Vel_State, Jerk_State {}
  ...
  public static boolean isDoubleConvertable(StateValue value) {
    return value.getValue() instanceof Measure
        || value.getValue() instanceof Double
        || value.getValue() instanceof DoubleSupplier
        || value.getValue() instanceof Float
        || value.getValue() instanceof Long
        || value.getValue() instanceof LongSupplier
        || value.getValue() instanceof Integer
        || value.getValue() instanceof IntSupplier;
  }

  public static double getValueAsDouble(StateValue value) {
    if (value.getValue() instanceof Measure) {
      if (value.getUnit() == null) {
        return ((Measure<?>) value.getValue()).magnitude();
      } else {
        return ((Measure<?>) value.getValue()).in(value.getUnit());
      }
    } else if (value.getValue() instanceof Double) {
      return (Double) value.getValue();
    } else if (value.getValue() instanceof DoubleSupplier) {
      return ((DoubleSupplier) value.getValue()).getAsDouble();
    } else if (value.getValue() instanceof Float) {
      return (Float) value.getValue();
    } else if (value.getValue() instanceof Long) {
      return (Long) value.getValue();
    } else if (value.getValue() instanceof LongSupplier) {
      return ((LongSupplier) value.getValue()).getAsLong();
    } else if (value.getValue() instanceof Integer) {
      return (Integer) value.getValue();
    } else if (value.getValue() instanceof DoubleSupplier) {
      return ((IntSupplier) value.getValue()).getAsInt();
    } else {
      throw new InvalidParameterException(
          value.getValue().toString() + " cannot be converted to a double.");
    }
  }

  @SuppressWarnings("unchecked")
  public static <U extends Unit> double getValueAsDouble(StateValue value, U unit) {
    UnaryFunction conversion = number -> number;
    if (value.getUnit() != null) {
      conversion = number -> unit.fromBaseUnits(value.getUnit().toBaseUnits(number));
    }
    if (value.getValue() instanceof Measure) {
      return ((Measure<U>) value.getValue()).in(unit);
    } else if (value.getValue() instanceof Double) {
      return conversion.apply((Double) value.getValue());
    } else if (value.getValue() instanceof DoubleSupplier) {
      return conversion.apply(((DoubleSupplier) value.getValue()).getAsDouble());
    } else if (value.getValue() instanceof Float) {
      return conversion.apply((Float) value.getValue());
    } else if (value.getValue() instanceof Long) {
      return conversion.apply((Long) value.getValue());
    } else if (value.getValue() instanceof LongSupplier) {
      return conversion.apply(((LongSupplier) value.getValue()).getAsLong());
    } else if (value.getValue() instanceof Integer) {
      return conversion.apply((Integer) value.getValue());
    } else if (value.getValue() instanceof DoubleSupplier) {
      return conversion.apply(((IntSupplier) value.getValue()).getAsInt());
    } else {
      throw new InvalidParameterException(
          value.getValue().toString() + " cannot be converted to a double.");
    }
  ...
  // public static class LinearP_State implements Linear, Vel_State {
  ...
  // //   public Unit[] getUnits() {
  // //     return new Unit[] {
  // //       Meters, MetersPerSecond, MetersPerSecondPerSecond,
  // MetersPerSecondPerSecond.per(Second)
  // //     };
  // //   }
  ...
  // public static class AngularP_State implements Angular, Vel_State {
  ...
  // // public record AngularAJ_State(double radPs2, double radPs3) implements Angular,
  // AccJerk_State {
  // //   @Override
  // //   public Unit[] getUnits() {
