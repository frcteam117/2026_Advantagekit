 ...
 */
public class ModifiedSwerveSetpointGenerator {
   ...
   *     per second
   */
  public ModifiedSwerveSetpointGenerator(
      RobotConfig config, Translation2d centerOfMass, double maxSteerVelocityRadsPerSec) {
    this.config = config;
   ...
   */
  public ModifiedSwerveSetpointGenerator(
      RobotConfig config, Translation2d centerOfMass, AngularVelocity maxSteerVelocity) {
    this(config, centerOfMass, maxSteerVelocity.in(RadiansPerSecond));
  }
  ...
  public SwerveSetpoint generateSetpoint(
    ...
    for (int m = 0; m < config.numModules; m++) {
      prev_vx[m] = prevSetpoint.moduleStates()[m].angle.getCos()
          * prevSetpoint.moduleStates()[m].speedMetersPerSecond;
      prev_vy[m] = prevSetpoint.moduleStates()[m].angle.getSin()
          * prevSetpoint.moduleStates()[m].speedMetersPerSecond;
      ...
      if (epsilonEquals(prevSetpoint.moduleStates()[m].speedMetersPerSecond, 0.0)) {
        ...
        if (numStepsNeeded <= 1.0) {
          ...
          overrideSteering.set(
              m,
              Optional.of(prevSetpoint.moduleStates()[m].angle.rotateBy(Rotation2d.fromRadians(
    ...
    for (int m = 0; m < config.numModules; m++) {
      double lastVelRadPerSec = prevSetpoint.moduleStates()[m].speedMetersPerSecond
    ...
    if (constraints != null) {
      ...
      chassisAngularAccel = MathUtil.clamp(
          chassisAngularAccel,
    ...
    // TODO: make this use center of mass
    ...
    // TODO: make this use the center of mass
