...
public class PathCommands {
  ...
  public Double getDistanceFromHub(DrivetrainSubsystem drivetrain) {
    ...
    double distance = Math.sqrt(
        (xDistance * xDistance) + (yDistance * yDistance)); // PYTHAGOREANS THEOREMMM (tho he def
  ...
  public void AlignToTag(
    ...
    //    change 2??? vvv
    var cameraData = vision.getCameraResults(camera1); // add back camera0
    ...
    if (true
        && targetVisible) { // reset the camera photonvision values so the targetrange stuff can be
      // accurate?
      ...
      /// -m_xspeedLimiter.calculate(MathUtil.applyDeadband(targetRange * 0.5, 0.03)) // CONFIGURE
      // STUFF SO U CAN TEST IF TS WORKS W/ SWERVE!!!!!
      // * SwerveConstants.TOP_SPEED_METERS_PER_SEC;
      double ySpeed = 0;
      // -m_yspeedLimiter.calculate(MathUtil.applyDeadband(targetYaw * kPVision_Turn, 0.03))
      // * //SwerveConstants.TOP_SPEED_METERS_PER_SEC;
      // SmartDashboard.putBoolean("DriveCommands.joystickDrive",true);
      DriveCommands.joystickDrive(drivetrain, () -> xSpeed, () -> ySpeed, () -> rot);
      // should this not be joystickDrive?
    }
  }
  ...
  public drivetrainValues getDrivetrainValues() {
    return new drivetrainValues(
        drivetrain_xSpeed, drivetrain_ySpeed, drivetrain_rot, drivetrain_fieldRelative);
  }
  ...
  public boolean CloseEnough(
      Pose2d curPose, Pose2d targetPose) { // gotta be a better way 2 do this but again idfk
  ...
  public Command DriveToCenterFromOrigin(
      DrivetrainSubsystem drivetrain,
